...
class MLP_simple(nn.Module):
    ...
    def __init__(self,drug_latent_size, ge_latent_size):
        super(MLP_simple, self).__init__()

        activateLayer = nn.PReLU()

        self.geMLP = nn.Sequential(
            nn.Linear(ge_latent_size, 256),
        ...
        self.dMLP = nn.Sequential(
            nn.Linear(drug_latent_size, 128),
...
class MLP_w_mutation(nn.Module):

    def __init__(self,drug_latent_size, ge_latent_size, mutations_size):
        super(MLP_w_mutation, self).__init__()
        ...
        self.mutMLP = nn.Sequential(
            nn.Linear(mutations_size, 256),
    ...
    def forward(self, geLatentVec, dLatentVec, mutations):
        ge = self.geMLP(geLatentVec)
        d = self.dMLP(dLatentVec)
        m = self.mutMLP(mutations)

        combination = torch.cat([ge, d, m], dim=1)
        res = self.combineMLP(combination)
